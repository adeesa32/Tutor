...
export class Activity1Component implements OnInit {
  ...
  getCuestionario(): void {
    this.apiservice.getAllCuestionario().subscribe((res: any) => {
      let cuestionario = [];
      for(let i = 0; i < res.problema.length; i++){
        let preguntaObject = {descripcion: res.problema[i].descripcion,
          preguntas: res.problema[i].preguntas,
          respuestas: res.problema[i].respuestas
        };
